...
async function saveRawJson (data) {
 ...
 async function sendTgMessage(data) {
  ...
  const text = data.splice(1,20).map((o, i) => {
    ...
    if (ranks[i]) {
      return `${ranks[i]} [_${o.title}_](https://hu60.cn/q.php/bbs.topic.${o.id}.html) ${(o.read_count / 10000).toFixed(2)} 万`
    }
    return `🔥 [-${o.title}_](https://hu60.cn/q.php/bbs.topic.${o.id}.html) ${(o.read_count / 10000).toFixed(2)} 万`})
