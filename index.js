...
async function saveRawJson (data) {
 ...
 async function sendTgMessage(data) {
  ...
  const text = data.splice(1,20).map((o, i) => {
    if (o.essence === 1) {
      return `🔥${ranks[i]}:<a href="https://hu60.cn/q.php/bbs.topic.${o.id}.html">${o.title}</a> @${o.uinfo.name},(${(o.read_count / 1000).toFixed(3)}k)``${moment().startOf('o.ctime').fromNow()}`
    }
    if (ranks[i]) {
      return `${ranks[i]}:<a href="https://hu60.cn/q.php/bbs.topic.${o.id}.html">${o.title}</a> @${o.uinfo.name},(${o.read_count})``${moment().startOf('o.ctime').fromNow()}`
    }
    return `:<a href="https://hu60.cn/q.php/bbs.topic.${o.id}.html">${o.title}</a> @${o.uinfo.name},(${o.read_count})``${moment().startOf('o.ctime').fromNow()}`
  })
  text.unshift(`${moment().startOf('o.ctime').fromNow()}的虎绿林首页存档${dayjs().format('YYYY-MM-DD HH:MM:ss')}`)
